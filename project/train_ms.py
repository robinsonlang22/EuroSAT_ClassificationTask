...
from config import MS_DATASET_ROOT, SPLITS_ROOT, PROJECT_ROOT, SEED
...
class EuroSATMSDataset(Dataset):
    ...
    def __init__(self, dataset_root: Path, project_root: Path, split: str, transform=None):
        ...
        split_path = project_root / f"{split}.txt"
...
def main():
    ...
    dummy_train_dataset = EuroSATMSDataset(
        DATASET_ROOT, SPLITS_ROOT, split="train", transform=eval_transform
    )
    ...
    val_dataset = EuroSATMSDataset(
        DATASET_ROOT, SPLITS_ROOT, split="val", transform=eval_transform
    )
    ...
    for aug_name in augmentation_setups:
        ...
        train_dataset = EuroSATMSDataset(
            DATASET_ROOT, SPLITS_ROOT, split="train", transform=train_transform
        )

...
def main(args):
    # run on val data to help improve model
    ...
    correct, incorrect = collect_results(model, val_loader, class_names)
    
    correct.sort(key=lambda x: x['conf'], reverse=True)
    incorrect.sort(key=lambda x: x['conf'], reverse=True)
    
    print("\n Generating Plots on Validation Set...")
    plot_top_bottom(correct[:5], "Top 5 Correct (Val Set)", "top5")
    plot_top_bottom(incorrect[:5], "Bottom 5 Failures (Val Set)", "bottom5")
